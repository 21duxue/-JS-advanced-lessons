...
console.log(o3.propertyIsEnumerable("x"));//false  x不可枚举

for(var k in o3){ //遍历不到x属性  x不可枚举
    console.log(k,o3[k]);
}
console.log("x" in o3,"y" in o3);// in 和 for...in 的区别 in可以找到不可枚举的值
...
for(var k in o5){//for...in 遍历一个对象的属性，包括继承的属性，但不包括不可枚举的属性 只有a c
    if(o5.hasOwnProperty(k)){
        console.log("o5 自有可遍历属性：",k,o5[k]);//c
    }else {
        console.log("o5 非自有可遍历属性：",k,o5[k]);//a
    }
}
console.log(o5.propertyIsEnumerable("a"),//false //判断自身是否有该属性并检测该属性是否可枚举 自身有切可枚举的只有c
    o5.propertyIsEnumerable("b"),//false
    o5.propertyIsEnumerable("c"),//true
    o5.propertyIsEnumerable("d")//false
);
//in  检测一个对象是否有某个属性，包括继承的属性，包括不可枚举的属性
console.log("a" in o5,"b" in o5,"c" in o5,"d" in o5);//全部为true
